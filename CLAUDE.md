...
Or double-click a repo-root launcher (self-healing: makes the `.venv` with uv, installs deps, runs):
`Run VAbk-Studio.bat` (console, for logs) or `Run VAbk-Studio.exe` (windowless via `pythonw`). The exe
is a native launcher compiled from `build/launcher/VAbkLauncher.cs` with the .NET Framework `csc` —
separate from the PyInstaller build, and it runs the **source**, not a frozen copy.
...
## Icon & versioning
- `app/__init__.py` `__version__` is the single source of truth: it shows in the window title and the
  PyInstaller spec generates a Windows version resource from it (`build/_version_info.txt`, gitignored)
  for the frozen exe's file properties. Bump it, then push a `v*` tag to cut a release
  (`release.yml`); `ci.yml` runs the offscreen `--smoke` on every push/PR.
- `build/VAbkStudio.ico` is generated by `build/launcher/make_icon.py` (renders an inline SVG via the
  venv PyQt6 — no extra deps), embedded in the frozen exe via the spec's `icon=` and in
  `Run VAbk-Studio.exe` via `csc /win32icon`. Regenerate it after editing the SVG.
